...
static TaskHandle_t g_xTaskHandleCar2;
static TaskHandle_t g_xTaskHandleCar3;
        ...
        //{
            if (pcar->x < g_xres - CAR_LENGTH)
            {
                ...
                if (pcar->x == g_xres - CAR_LENGTH)
                {
                    ...
                    xTaskNotify(g_xTaskHandleCar3, 111, eSetValueWithOverwrite);
...
void Car3Task(void *params)
{
    ...
    while (pulNotificationValue != 111);
...
{    
    ...
    xTaskCreate(Car2Task, "car2", 128, &g_cars[1], osPriorityNormal+2, &g_xTaskHandleCar2);
    xTaskCreate(Car3Task, "car3", 128, &g_cars[2], osPriorityNormal+2, &g_xTaskHandleCar3);
}