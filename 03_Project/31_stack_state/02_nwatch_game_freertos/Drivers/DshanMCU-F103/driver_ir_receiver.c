...
/* 红外队列，不再定义为外部的，而是定义为自己的 */
static QueueHandle_t g_xQueueIR;

static QueueHandle_t g_xQueueRegister[10];
static int g_queue_register_count;
...
/**
  ...
  * @retval 无
  */
void RegisterQueueHandle(QueueHandle_t handle)
{
    if (g_queue_register_count < 10)
    {
        g_xQueueRegister[g_queue_register_count] = handle;
        g_queue_register_count++;
    }
}

/**
  * @brief 分发按键数据
  * @param  pidata idata指针
  * @retval 无
  */
void DispatchKey(struct ir_data *pidata)
{
    ...
    int i;
    for(i = 0; i < g_queue_register_count; i++)
    {
        xQueueSendToBackFromISR(g_xQueueRegister[i], pidata, &xHigherPriorityTaskWoken);  
    }
